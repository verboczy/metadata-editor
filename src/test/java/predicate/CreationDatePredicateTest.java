...
public class CreationDatePredicateTest {

    private static final File txtFile = new File("src/test/resources/predicate/example.txt"); // Created on 2021-03-12

    //region Arguments
    private static Stream<Arguments> createDateArguments() {
        // @formatter:off
        return Stream.of(
    ...
    //region  Test
    @DisplayName("Creation date predicate tests")
    @ParameterizedTest(name = "{index} - {0}, expected: {4}")
    @MethodSource("createDateArguments")
    void creationDatePredicateTest(@SuppressWarnings("unused") final String name, final boolean enabled, final LocalDate afterDate, final LocalDate beforeDate, final boolean expected) {
        // Given
        final CreationDatePredicate creationDatePredicate = new CreationDatePredicate(enabled, afterDate, beforeDate);

        // When
        final boolean actual = creationDatePredicate.getPredicate().test(txtFile);
